...
export function MapZoomControls({ scale, maxScale, zoomed, onZoomIn, onZoomOut, onReset, className }: Props) {
  return (
    <div className={`flex flex-col items-end gap-1.5 ${className ?? ""}`}>
      {/* 배지는 버튼 위에 둔다: 나타나고 사라져도 ＋/－ 위치가 그대로 */}
      {zoomed ? (
        <button
          type="button"
          onClick={onReset}
        ...
        >
          {scale >= 10 ? scale.toFixed(0) : scale.toFixed(1)}× · 전체 보기
        </button>
      ) : null}
      <div className="flex flex-col overflow-hidden rounded-[10px] border border-line shadow-sm" role="group" aria-label="지도 확대">
        <button type="button" onClick={onZoomIn} className={btn} aria-label="확대" disabled={scale >= maxScale - 0.01}>
