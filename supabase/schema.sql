-- 나의 일본 여행 지도 — Supabase schema v0.3 (현재 전체 스키마)
-- 변경 이력은 supabase/migrations/ 에 새 파일로. 이 파일은 항상 현재 상태를 반영한다.
-- 원칙: 개인용, 로그인 없음. 읽기는 anon(공개 select), 쓰기는 서버의 service_role 로만.
--       anon 에게 insert/update/delete 정책을 절대 주지 않는다.
...
create index on visits (city_id);
...
-- ---------- Storage ----------
-- 버킷 trip-photos (private). 업로드·서명 URL 발급은 서버(service_role)에서만 → storage.objects 정책 없음.
insert into storage.buckets (id, name, public) values ('trip-photos', 'trip-photos', false) on conflict (id) do nothing;

-- ---------- 시드 ----------
-- supabase/seed.sql (또는 pnpm seed) → prefectures 47, cities 48
